...
export function ConfigurationGate({ children }: { children: ReactNode }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-canvas px-5 text-ink">
      <section
        className="w-full max-w-2xl rounded-[16px] border border-ruby/30 bg-panel p-7"
        role="alert"
      ...
      >
        ...
        <ul className="mt-5 space-y-2 rounded-[10px] border border-white/[0.08] bg-canvas p-4 font-mono text-[12px] text-ink-soft">
          {errors.map((error) => (
