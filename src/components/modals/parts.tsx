...
export function ModalGemHeader({ gem }: { gem: DecoratedGem }) {
  return (
    <div className="flex items-center gap-3 rounded-[12px] border border-white/[0.08] bg-panel p-3">
      <GemThumb gem={gem} height={44} rounded="rounded-[10px]" showTag={false} showCarat={false} className="w-11 shrink-0" />
...
export function SummaryRow({ label, value, accent }: { label: string; value: React.ReactNode; accent?: string }) {
  return (
...
export function assetAmountPreview(usd: number, asset?: PaymentAsset): string {
  ...
  const formatted = asset.isNative ? amt.toFixed(4) : amt.toLocaleString('en-US', { maximumFractionDigits: 2 });
